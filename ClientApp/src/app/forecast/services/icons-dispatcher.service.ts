 ...
 */
@Injectable()
export class IconsDispatcherService
{

  isNight: boolean = false;

  getUrl( id: string, size: number=1){
    return 'http://openweathermap.org/img/wn/'+id+'@'+(size*2)+'x.png';
  }

  getClearSkyUrl(size: number=1){
    return this.getUrl('01'+(this.isNight?'n':'d'),size);
  }
  getFewCloudsUrl(size: number=1){
    return this.getUrl('02'+(this.isNight?'n':'d'),size);
  }
  getScatteredSkyUrl(size: number=1){
    return this.getUrl('03'+(this.isNight?'n':'d'),size);
  }
  getBrokenSkyUrl(size: number=1){
    return this.getUrl('04'+(this.isNight?'n':'d'),size);
  }
  getShowerRainUrl(size: number=1){
    return this.getUrl('09'+(this.isNight?'n':'d'),size);
  }
  getRainUrl(size: number=1){
    return this.getUrl('10'+(this.isNight?'n':'d'),size);
  }
  getThunderstormUrl(size: number=1){
    return this.getUrl('11'+(this.isNight?'n':'d'),size);
  }
  getSnowUrl(size: number=1){
    return this.getUrl('13'+(this.isNight?'n':'d'),size);
  }
  getMistUrl(size: number=1){
    return this.getUrl('50'+(this.isNight?'n':'d'),size);
  }
